...
def moo_crypt(text: str, salt: str = '') -> str:
    """
    Hash a password using a MOO-compatible scheme.

    Produces a salted SHA-256 hash in the format ``$<salt>$<hex_digest>``.
    If no *salt* is provided, a cryptographically random 16-character
    alphanumeric salt is generated automatically.

    This replaces the classic Unix ``crypt(3)`` used by older MOO servers
    with a stronger hash function while maintaining a simple, portable
    format.

    Args:
        text (str): The plaintext password to hash.
        salt (str): An explicit salt string.  If empty (the default), a
            random salt is generated.

    Returns:
        str: The hashed password in ``$<salt>$<hex_digest>`` format.

    Note:
        The salt is prepended to the plaintext before hashing, so the
        same plaintext with different salts produces different digests.
    """
    if not salt:
        salt = ''.join(random.choices(string.ascii_letters + string.digits, k=16))

    hash_obj = hashlib.sha256((salt + text).encode('utf-8'))
    hash_hex = hash_obj.hexdigest()

    return f"${salt}${hash_hex}"


def verify_crypt(text: str, hashed: str) -> bool:
    """
    Verify a plaintext password against a hashed password.

    Extracts the salt from the ``$<salt>$<hex_digest>`` format produced
    by :func:`moo_crypt`, re-hashes *text* with that salt, and compares
    the result.

    Args:
        text (str):   The plaintext password to verify.
        hashed (str): The hashed password (from :func:`moo_crypt`).

    Returns:
        bool: ``True`` if *text* matches the hash, ``False`` otherwise.
            Also returns ``False`` if *hashed* is not in the expected format.
    """
    # Validate format: must start with '$' and have exactly 3 parts
    # when split on '$' (empty prefix, salt, hex digest)
    ...
    # Re-hash with the extracted salt and compare
    actual = moo_crypt(text, salt)
