"""
do_paralyze -- the 'paralyze' effect, fired by $eu once per tick.

Keeps status['paralyzed'] at the number of ticks left, which is the shape the
readers already expect: do_wait, get_status and make_postatus all ask
`.get('paralyzed', 0)` and treat any positive number as afflicted. The key is
removed on the last tick rather than left sitting at zero.

$eu dispatches by name -- it calls do_paralyze because the effect is called
'paralyze' -- so this file *is* the binding. There is no table of names to
