"""
do_imprison -- the 'imprison' effect, fired by $eu once per tick.

Keeps condition['imprisoned'] at the number of ticks left, which is the shape the
readers already expect: do_wait, get_condition and make_postatus all ask
`.get('imprisoned', 0)` and treat any positive number as afflicted. The key is
removed on the last tick rather than left sitting at zero.

$eu dispatches by name -- it calls do_imprison because the effect is called
'imprison' -- so this file *is* the binding. There is no table of names to
