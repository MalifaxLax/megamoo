...
if wrong key.
"""
if not getattr(this, 'clock', 0):
    player.msg((getattr(this, 'culockf', '') or '%d is not locked.').replace('%d', this.noun or this.name))
    return
...
if key_obj is not None:
    ...
    if not iobj or (key_obj and iobj.objnum != key_obj.objnum):
        ...
        player.msg((getattr(this, 'ulockf', '') or "You can't unlock %d with %i because %i does not fit the lock.").replace('%d', this.noun or this.name).replace('%i', iname))
        return
this.set_property('clock', 0, db)
iname = iobj.noun or iobj.name if iobj else ''
player.msg(su.psub1((getattr(this, 'unlock', '') or 'You unlock %d.').replace('%d', this.noun or this.name).replace('%i', iname), player))
if not getattr(player, 'invis', False):
    omsg = getattr(this, 'ounlock', '')
    if omsg:
