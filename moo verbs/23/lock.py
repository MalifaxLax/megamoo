...
if getattr(this, 'clock', 0):
    player.msg((getattr(this, 'llock', '') or '%d is already locked!').replace('%d', this.noun or this.name))
    return
if not getattr(this, 'closed', 0):
    player.msg((getattr(this, 'clockf', '') or 'You need to close %d before you can lock it!').replace('%d', this.noun or this.name))
    return
key_obj = getattr(this, 'key', None)
if key_obj is not None:
    ...
    if not iobj or (key_obj and iobj.objnum != key_obj.objnum):
        ...
        player.msg((getattr(this, 'lockf', '') or "You attempt to lock %d with %i but %i doesn't fit the lock.").replace('%d', this.noun or this.name).replace('%i', iname))
        return
this.set_property('clock', 1, db)
player.msg(su.psub1((getattr(this, 'lock', '') or 'You lock %d.').replace('%d', this.noun or this.name), player))
